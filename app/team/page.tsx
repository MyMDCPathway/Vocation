...
export default function MeetTheTeam() {
  return (
    <div className="flex min-h-screen flex-col bg-surface">
      ...
      <main className="mx-auto w-full max-w-[1000px] flex-1 px-5 py-14 md:px-16">
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-3">
          {TEAM.map((member) => (
            <div
              key={member.name}
              className="flex flex-col items-center rounded-xl bg-surface-lowest p-6 text-center shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-lift"
            >
              <img
                src={member.image}
                alt={member.name}
              ...
              />
              <p className="mt-4 font-semibold text-on-surface">{member.name}</p>
        ...
        <p className="mx-auto mt-12 max-w-2xl text-on-surface-variant">
