...
export function CostPanel({
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        <dl className="mt-6 space-y-3 border-t border-gray-100 pt-6 text-sm">
          <div className="flex items-baseline justify-between gap-4">
            <dt className="text-gray-600">Time to qualified</dt>
            <dd className="font-semibold text-gray-900">
              ~{cost.years} {cost.years === 1 ? "year" : "years"}
            </dd>
          </div>
          <div className="flex items-baseline justify-between gap-4">
            <dt className="text-gray-600">Grant aid outlook</dt>
      ...
      <div className="relative overflow-hidden rounded-xl border border-gray-200 bg-gradient-to-b from-white to-gray-50 p-6">
        ...
        <ul className="mt-5 space-y-3">
          {LOCKED_ROWS.map((row) => (
            <li key={row.label} className="flex gap-3">
              <span
                aria-hidden="true"
                className="mt-1 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-gray-200"
              >
        ...
        <div className="mt-6 rounded-lg border border-dashed border-gray-300 bg-white p-4">
          ...
          <p className="mt-1 text-xs leading-relaxed text-gray-500">{aid.detail}</p>
        </div>
